...
sequenceDiagram
  ...
  participant Userinfo as /api/oauth/userinfo

  App->>User: Redirect to consent URL
  User->>Consent: Sign in and approve
  Consent->>App: Redirect with code
  App->>Token: Exchange code (+ client secret)
  Token->>App: access_token (+ refresh if offline_access)
  App->>Userinfo: Fetch profile
