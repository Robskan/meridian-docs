...
<Callout icon="info" color="#6366f1">
  **Testing older flows:** Runs require **graphJson v3** (`v: 3`). v1–v2 graphs fail until the flow is saved again; the builder migrates on save when you open a legacy flow (no in-app resave banner yet). Open and **save once** so the flow upgrades—otherwise run fails.
</Callout>
