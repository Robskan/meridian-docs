...
On the text, button and select menu components, emojis can be added using an emoji selector. We're using the same emojis as Discord for your convenience. In addition, the emoji selector is also capable of displaying your bot emojis and emojis from servers your bot is in.
...
<Columns cols={2}>
  <Column>
    ...
    In the same Action dropdown, select ''Direct Message'' to send the message to any applicable user's private messages as the bot.<br /><br />After selecting Direct Message in the main Action dropdown, a new Source dropdown appears, allowing you to additionally choose if you want to send the Direct Message to:
...
| <Badge color="red" stroke>Red</Badge> | This specific component can't be placed in this position. Move it elsewhere or release it here to cancel its placement. |

When attempting to place a component in an invalid position, the blue area will become red and an error message will slide in from the bottom of the message editor to explain why that is an invalid position.
